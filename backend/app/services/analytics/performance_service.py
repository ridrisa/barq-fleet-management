...
class PerformanceService(CRUDBase[PerformanceData, PerformanceCreate, PerformanceUpdate]):
    ...
    def get_by_courier(
        ...
        *,
        ...
        return self.get_multi(
            db, skip=skip, limit=limit, filters={"courier_id": courier_id}, order_by=order_by
        )

    def get_by_date(self, db: Session, performance_date: date) -> List[PerformanceData]:
        """Get all performance records for a specific date"""
        return db.query(PerformanceData).filter(PerformanceData.date == performance_date).all()
    ...
    def get_by_date_range(
        ...
        *,
        ...
        query = db.query(PerformanceData).filter(
            and_(PerformanceData.date >= start_date, PerformanceData.date <= end_date)
        )
    ...
    def get_by_courier_and_date(
        self, db: Session, courier_id: int, performance_date: date
    ...
    def calculate_metrics(
        self, db: Session, courier_id: int, start_date: date, end_date: date
    ) -> PerformanceStats:
        """Calculate performance metrics for a courier within date range"""
        records = self.get_by_date_range(
            db, start_date=start_date, end_date=end_date, courier_id=courier_id
        )

        if not records:
            return PerformanceStats(
                courier_id=courier_id, period_start=start_date, period_end=end_date
            )
        ...
        avg_rating = (
            sum(r.average_rating for r in records) / working_days
            if working_days > 0
            else Decimal("0.0")
        )
        avg_efficiency = (
            sum(r.efficiency_score for r in records) / working_days
            if working_days > 0
            else Decimal("0.0")
        )
        avg_orders_per_day = (
            Decimal(str(total_completed)) / working_days if working_days > 0 else Decimal("0.0")
        )
        ...
        *,
        ...
        query = (
            db.query(
                ...
                func.sum(PerformanceData.orders_completed + PerformanceData.orders_failed).label(
                    "total_all_orders"
                ),
            )
            .join(Courier, PerformanceData.courier_id == Courier.id)
            .filter(and_(PerformanceData.date >= start_date, PerformanceData.date <= end_date))
            .group_by(PerformanceData.courier_id, Courier.barq_id, Courier.full_name)
        )
        ...
        for rank, result in enumerate(results, start=1):
            on_time_rate = (
                (float(result.total_on_time) / float(result.total_orders) * 100)
    ...
    def compare_couriers(
        self, db: Session, courier_ids: List[int], start_date: date, end_date: date
    ...
    def get_statistics(self, db: Session) -> Dict[str, Any]:
        ...
        total_orders = db.query(func.sum(PerformanceData.orders_completed)).scalar() or 0

        total_revenue = db.query(func.sum(PerformanceData.revenue_generated)).scalar() or Decimal(
            "0.0"
        )

        avg_efficiency = db.query(func.avg(PerformanceData.efficiency_score)).scalar() or Decimal(
            "0.0"
        )

        avg_rating = db.query(func.avg(PerformanceData.average_rating)).scalar() or Decimal("0.0")
    ...
    def upsert_performance(
        self, db: Session, courier_id: int, performance_date: date, data: PerformanceCreate
    ) -> PerformanceData:
