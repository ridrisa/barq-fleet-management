...
class DispatchAssignmentService(
    ...
    """Service for dispatch assignment management operations"""
        ...
        *,
        ...
        if filters:
            for key, value in filters.items():
                if hasattr(DispatchAssignment, key) and value is not None:
                    query = query.filter(getattr(DispatchAssignment, key) == value)
        return (
            query.order_by(DispatchAssignment.created_at_time.desc())
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def get_pending(
        ...
        query = db.query(DispatchAssignment).filter(
            DispatchAssignment.status == DispatchStatus.PENDING
        )
        if organization_id:
            query = query.filter(DispatchAssignment.organization_id == organization_id)
        return (
...
dispatch_assignment_service = DispatchAssignmentService(DispatchAssignment)
