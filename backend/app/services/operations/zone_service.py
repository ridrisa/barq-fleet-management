...
class ZoneService(CRUDBase[Zone, ZoneCreate, ZoneUpdate]):
    ...
    def get_by_code(
        self, db: Session, *, zone_code: str, organization_id: int = None
    ) -> Optional[Zone]:
        """Get zone by unique code"""
        query = db.query(Zone).filter(Zone.zone_code == zone_code)
        if organization_id:
            query = query.filter(Zone.organization_id == organization_id)
        return query.first()
    ...
    def get_active_zones(
        ...
        query = db.query(Zone).filter(Zone.status == ZoneStatus.ACTIVE)
        if organization_id:
            query = query.filter(Zone.organization_id == organization_id)
        return query.offset(skip).limit(limit).all()

    def get_by_city(
        self, db: Session, *, city: str, skip: int = 0, limit: int = 100, organization_id: int = None
    ) -> List[Zone]:
        """Get zones by city"""
        query = db.query(Zone).filter(Zone.city == city)
        if organization_id:
            query = query.filter(Zone.organization_id == organization_id)
        return query.offset(skip).limit(limit).all()

    def get_at_capacity(self, db: Session, *, organization_id: int = None) -> List[Zone]:
        """Get zones at or near capacity"""
        query = db.query(Zone).filter(
            Zone.status == ZoneStatus.ACTIVE, Zone.current_couriers >= Zone.max_couriers
        )
        if organization_id:
            query = query.filter(Zone.organization_id == organization_id)
        return query.all()
