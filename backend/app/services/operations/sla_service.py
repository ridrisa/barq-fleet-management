...
class SLADefinitionService(CRUDBase[SLADefinition, SLADefinitionCreate, SLADefinitionUpdate]):
    """Service for SLA definition management operations"""
...
class SLATrackingService(CRUDBase[SLATracking, SLATrackingCreate, SLATrackingUpdate]):
    """Service for SLA tracking management operations"""
    ...
    def get_at_risk(
        ...
        if organization_id:
            query = query.filter(SLATracking.organization_id == organization_id)
        return (
            query.order_by(SLATracking.target_completion_time.asc())
            .offset(skip)
            .limit(limit)
            .all()
        )
...
sla_definition_service = SLADefinitionService(SLADefinition)
sla_tracking_service = SLATrackingService(SLATracking)
