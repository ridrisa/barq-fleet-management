...
class IncidentService(CRUDBase[Incident, IncidentCreate, IncidentUpdate]):
    ...
    def get_by_type(
        self, db: Session, *, incident_type: IncidentType, skip: int = 0, limit: int = 100
    ) -> List[Incident]:
        """
    ...
    def get_by_date_range(
        ...
        *,
        ...
        query = db.query(self.model).filter(
            and_(self.model.incident_date >= start_date, self.model.incident_date <= end_date)
        )
        ...
        return query.order_by(self.model.incident_date.desc()).offset(skip).limit(limit).all()
    ...
    def get_open_incidents(
        ...
        *,
        ...
        """
        query = db.query(self.model).filter(
            self.model.status.in_([IncidentStatus.REPORTED, IncidentStatus.INVESTIGATING])
        )
    ...
    def resolve_incident(
        self, db: Session, *, incident_id: int, resolution: str, cost: Optional[int] = None
    ) -> Optional[Incident]:
        """
        ...
        update_data = {"status": IncidentStatus.RESOLVED, "resolution": resolution}
    ...
    def close_incident(
        self, db: Session, *, incident_id: int, resolution: Optional[str] = None
    ) -> Optional[Incident]:
        """
        Close an incident
    ...
    def get_statistics(
        ...
        *,
        ...
        return {
            ...
            "resolution_rate": (
                (resolved + closed) / total_incidents * 100 if total_incidents > 0 else 0
            ),
        }

    def get_high_cost_incidents(
        self, db: Session, *, min_cost: int = 1000, skip: int = 0, limit: int = 100
    ) -> List[Incident]:
        """
    ...
    def get_recent_incidents(
        ...
        *,
        ...
        query = db.query(self.model).filter(self.model.incident_date >= cutoff_date)
