...
def generate_monthly_reports():
    ...
    try:
        ...
        # Format and email reports to management
        ...
        <html>
        <body>
            <h1>BARQ Fleet Management - Monthly Report</h1>
            <p>Report Period: {last_month_start.strftime('%B %Y')}</p>

            <h2>Fleet Status Summary</h2>
            <ul>
                <li><strong>Total Couriers:</strong> {fleet_report.get('total_couriers', 'N/A')}</li>
                <li><strong>Active Couriers:</strong> {fleet_report.get('active_couriers', 'N/A')}</li>
                <li><strong>Total Vehicles:</strong> {fleet_report.get('total_vehicles', 'N/A')}</li>
                <li><strong>Active Vehicles:</strong> {fleet_report.get('active_vehicles', 'N/A')}</li>
            </ul>

            <h2>Payroll Summary</h2>
            <ul>
                <li><strong>Total Payroll:</strong> SAR {payroll_report.get('total_amount', 0):,.2f}</li>
                <li><strong>Employees Processed:</strong> {payroll_report.get('employee_count', 'N/A')}</li>
            </ul>

            <h2>Leave Analytics</h2>
            <ul>
                <li><strong>Total Leave Requests:</strong> {hr_report.get('total_requests', 'N/A')}</li>
                <li><strong>Approved:</strong> {hr_report.get('approved', 'N/A')}</li>
                <li><strong>Pending:</strong> {hr_report.get('pending', 'N/A')}</li>
            </ul>

            <p style="color: #666; font-size: 12px;">
                This is an automated report generated by BARQ Fleet Management System.
            </p>
        </body>
        </html>
        """

        # Send to management (configure management emails in environment)
        import os
        management_emails = os.getenv("MANAGEMENT_REPORT_EMAILS", "").split(",")
        management_emails = [e.strip() for e in management_emails if e.strip()]

        if management_emails:
            recipients = [EmailRecipient(email=email) for email in management_emails]
            email_service.send_bulk_notification(
                recipients=recipients,
                subject=f"BARQ Fleet - Monthly Report {last_month_start.strftime('%B %Y')}",
                html_content=html_content,
            )
            logger.info(f"Monthly report sent to {len(management_emails)} recipients")
        else:
            logger.warning("No management emails configured - report not sent")
