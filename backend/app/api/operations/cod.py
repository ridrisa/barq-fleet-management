...
from app.schemas.operations.cod import CODCreate, CODResponse, CODUpdate
...
def get_cod_transaction(
    cod_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)
):
    """Get a specific COD transaction by ID"""
    cod = crud_cod.get(db, id=cod_id)
    if not cod:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="COD transaction not found"
        )
    return cod
...
def create_cod_transaction(
    cod_in: CODCreate, db: Session = Depends(get_db), current_user=Depends(get_current_user)
):
