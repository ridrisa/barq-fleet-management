...
def get_vehicle(
    vehicle_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)
):
...
def create_vehicle(
    vehicle_in: VehicleCreate, db: Session = Depends(get_db), current_user=Depends(get_current_user)
):
...
def get_vehicle_history(
    ...
    maintenance_records = sorted(
        vehicle.maintenance_records, key=lambda x: x.created_at or x.id, reverse=True
    )
    fuel_logs = sorted(vehicle.fuel_logs, key=lambda x: x.created_at or x.id, reverse=True)
    accident_logs = sorted(vehicle.accident_logs, key=lambda x: x.created_at or x.id, reverse=True)
    vehicle_logs = sorted(vehicle.vehicle_logs, key=lambda x: x.created_at or x.id, reverse=True)
    assignments = sorted(
        vehicle.assignment_history, key=lambda x: x.created_at or x.id, reverse=True
    )
