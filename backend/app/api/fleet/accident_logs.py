...
from app.schemas.fleet.accident_log import AccidentLogCreate, AccidentLogResponse, AccidentLogUpdate
...
def get_accident_log(
    accident_log_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)
):
    """Get a specific accident log by ID"""
    accident_log = crud_accident_log.get(db, id=accident_log_id)
    if not accident_log:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Accident log not found")
    return accident_log
