...
from app.schemas.hr.salary import SalaryCreate, SalaryResponse, SalaryUpdate
...
def get_salary(
    salary_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)
):
...
def create_salary(
    salary_in: SalaryCreate, db: Session = Depends(get_db), current_user=Depends(get_current_user)
):
    """Create a new salary"""
...
def get_salary_history(
    courier_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)
):
