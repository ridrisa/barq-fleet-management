...
from app.schemas.hr.leave import LeaveCreate, LeaveResponse, LeaveUpdate
...
def get_leave(leave_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
...
def create_leave(
    leave_in: LeaveCreate, db: Session = Depends(get_db), current_user=Depends(get_current_user)
):
