...
def create_expense(db: Session = Depends(get_db), current_user=Depends(get_current_user)):
...
def update_expense(
    expense_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)
):
