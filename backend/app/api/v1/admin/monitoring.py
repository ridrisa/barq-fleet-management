...
def get_system_health(
    ...
    error_count = db.query(AuditLog).filter(
        ...
        AuditLog.action.in_(["error", "failed"])
    ).count()
    ...
    if not database_healthy or (cpu_percent and cpu_percent > 90) or (memory and memory.percent > 90):
...
def get_database_stats(
    ...
    # Get largest table
    try:
        largest_table_result = db.execute(text("""
            SELECT
                tablename,
...
def get_api_metrics(
    ...
    # Count requests
    requests_last_hour = db.query(AuditLog).filter(
        AuditLog.created_at >= one_hour_ago
    ).count()

    requests_last_day = db.query(AuditLog).filter(
    ...
    for resource, count in top_resources:
        if resource:
            top_endpoints.append({
                "endpoint": resource,
                "count": count,
                "percentage": round(count / requests_last_day * 100, 2) if requests_last_day > 0 else 0
            })
