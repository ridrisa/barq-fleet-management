...
from app.crud.operations.settings import (
    operations_settings, dispatch_rule, sla_threshold,
    notification_setting, zone_default
)
from app.schemas.operations.settings import (
    OperationsSettingsCreate, OperationsSettingsUpdate, OperationsSettingsResponse,
    SettingValue, SettingsGroup,
    DispatchRuleCreate, DispatchRuleUpdate, DispatchRuleResponse,
    SLAThresholdCreate, SLAThresholdUpdate, SLAThresholdResponse,
...
def list_settings(
    ...
    """
    if setting_group:
        settings = operations_settings.get_by_group(db, setting_group=setting_group)
    else:
        settings = operations_settings.get_active_settings(db)
    return settings
...
def list_setting_groups(
    ...
    return operations_settings.get_groups(db)
...
def get_settings_grouped(
    ...
    groups = operations_settings.get_groups(db)
    result = []
    for group in groups:
        settings = operations_settings.get_by_group(db, setting_group=group)
...
def get_setting_by_key(
    ...
    setting = operations_settings.get_by_key(db, setting_key=setting_key)
    if not setting:
        raise HTTPException(
...
def create_setting(
    ...
    Business Logic:
    - Validates setting key is unique
    - Sets value based on value_type
    - Validates against constraints if provided
    """
    existing = operations_settings.get_by_key(db, setting_key=setting_in.setting_key)
    if existing:
        raise HTTPException(
    ...
    setting = operations_settings.create(db, obj_in=setting_in)
    return setting
...
def set_setting_value(
    ...
    if not setting:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Setting not found or is read-only"
        )
...
def list_dispatch_rules(
    ...
    """
    if zone_id:
        rules = dispatch_rule.get_rules_for_zone(db, zone_id=zone_id)
    elif active_only:
        rules = dispatch_rule.get_active_rules(db)
    else:
        rules = dispatch_rule.get_multi(db, skip=0, limit=100)
    return rules
...
def get_dispatch_rule(
    ...
    if not rule:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Dispatch rule not found"
        )
    return rule


@router.post("/dispatch-rules", response_model=DispatchRuleResponse, status_code=status.HTTP_201_CREATED)
def create_dispatch_rule(
    ...
    rule = dispatch_rule.create(db, obj_in=rule_in)
    return rule
...
def list_sla_thresholds(
    ...
    """
    if sla_type:
        thresholds = sla_threshold.get_by_sla_type(db, sla_type=sla_type)
    elif service_type:
        thresholds = sla_threshold.get_by_service_type(db, service_type=service_type)
    elif zone_id:
        thresholds = sla_threshold.get_for_zone(db, zone_id=zone_id)
    else:
        thresholds = sla_threshold.get_active_thresholds(db)
    return thresholds
...
def get_sla_threshold(
    ...
    if not threshold:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="SLA threshold not found"
        )
    return threshold


@router.post("/sla-thresholds", response_model=SLAThresholdResponse, status_code=status.HTTP_201_CREATED)
def create_sla_threshold(
    ...
    Business Logic:
    - Validates threshold code is unique
    - Warning must be less than target
    - Critical must be greater than target
    - Can apply to specific zone or all zones
    """
    existing = sla_threshold.get_by_code(db, threshold_code=threshold_in.threshold_code)
    if existing:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    threshold = sla_threshold.create(db, obj_in=threshold_in)
    return threshold
...
def list_notification_settings(
    ...
    """
    if event_type:
        setting = notification_setting.get_by_event_type(db, event_type=event_type)
        return [setting] if setting else []
    else:
        return notification_setting.get_active_settings(db)
...
@router.post("/notifications", response_model=NotificationSettingResponse, status_code=status.HTTP_201_CREATED)
def create_notification_setting(
    ...
    """
    existing = notification_setting.get_by_code(db, setting_code=setting_in.setting_code)
    if existing:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    setting = notification_setting.create(db, obj_in=setting_in)
    return setting
...
def list_zone_defaults(
    ...
    return zone_default.get_active_defaults(db)
...
def get_default_zone_template(
    ...
    default = zone_default.get_default_template(db)
    if not default:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="No default zone template configured"
        )
    return default
...
def get_zone_default(
    ...
    if not default:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Zone default not found"
        )
    return default


@router.post("/zone-defaults", response_model=ZoneDefaultResponse, status_code=status.HTTP_201_CREATED)
def create_zone_default(
    ...
    """
    existing = zone_default.get_by_code(db, default_code=default_in.default_code)
    if existing:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    default = zone_default.create(db, obj_in=default_in)
    return default
