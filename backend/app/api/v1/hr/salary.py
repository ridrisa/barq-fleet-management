...
def create_salary(
    ...
    if existing:
        raise HTTPException(
            status_code=400,
            detail="Salary record already exists for this courier in this month"
        )

    return salary_service.create(db, obj_in=salary_in)
...
def calculate_salary(
    ...
    if existing:
        raise HTTPException(
            status_code=400,
            detail="Salary already calculated for this period"
        )
...
def get_salary_statistics(
    ...
    """
    return salary_service.get_statistics(db, month=month, year=year)
