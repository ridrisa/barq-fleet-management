...
def get_leaves(
    ...
    if status:
        return leave_service.get_by_status(db, status=status, skip=skip, limit=limit)

    # If courier_id filter is provided
    if courier_id:
        return leave_service.get_by_courier(db, courier_id=courier_id, skip=skip, limit=limit)
...
def get_leave_statistics(
    ...
    """
    return leave_service.get_statistics(db)
