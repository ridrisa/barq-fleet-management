...
class Dashboard(TenantMixin, BaseModel):
    ...
    layout = Column(
        JSONB, nullable=True, comment="Dashboard layout configuration (grid positions, etc.)"
    )
    ...
    is_default = Column(
        Boolean, default=False, nullable=False, comment="Is this the user's default dashboard?"
    )
    is_shared = Column(
        Boolean, default=False, nullable=False, comment="Is dashboard shared with team?"
    )

    # Configuration
    refresh_interval_seconds = Column(
        Integer, default=300, nullable=False, comment="Auto-refresh interval"
    )
