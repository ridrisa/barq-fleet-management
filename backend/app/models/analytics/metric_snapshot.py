...
class MetricSnapshot(BaseModel):
    ...
    metric_name = Column(String(100), nullable=False, index=True, comment="Name of the metric (e.g., 'active_couriers', 'deliveries_completed')")
    metric_type = Column(String(50), nullable=False, index=True, comment="Type: counter, gauge, histogram")

    # Metric value
    value = Column(Numeric(20, 4), nullable=False, comment="Metric value")

    # Dimensions for filtering and grouping
    dimensions = Column(JSONB, nullable=True, comment="Flexible dimensions (e.g., {'city': 'Riyadh', 'zone': 'North'})")

    # Timestamp
    timestamp = Column(DateTime(timezone=True), nullable=False, index=True, comment="When the metric was recorded")
