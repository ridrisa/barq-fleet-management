...
class ApiKeyBase(BaseModel):
    ...
    description: Optional[str] = Field(
        None, description="Detailed description of the API key's purpose"
    )
    scopes: List[str] = Field(
        default_factory=list, description="List of allowed scopes/permissions"
    )
    ...
    expires_at: Optional[datetime] = Field(
        None, description="Expiration date (None = no expiration)"
    )
